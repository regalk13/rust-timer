...
use crate::timer::{SQUARE_SIZE, TIMER_HEIGHT, TIMER_WIDTH};
...
mod timer {
    ...
    pub const TIMER_WIDTH: u32 = 49;
    pub const TIMER_HEIGHT: u32 = 40;
    ...
    pub struct GameOfLife {
        playground: [bool; (TIMER_WIDTH * TIMER_HEIGHT) as usize],
        state: State,
    }
    /*
    #[derive(Default)]
    ...
    impl GameOfLife {
        pub fn new() -> GameOfLife {
            let mut playground = [false; (TIMER_WIDTH * TIMER_HEIGHT) as usize];

            // let's make a nice default pattern !
            for i in 1..(TIMER_HEIGHT - 1) {
                playground[(1 + i * TIMER_WIDTH) as usize] = true;
                playground[((TIMER_WIDTH - 2) + i * TIMER_HEIGHT) as usize] = true;
            }
            for j in 2..(TIMER_WIDTH - 2) {
                playground[(TIMER_WIDTH + j) as usize] = true;
                playground[((TIMER_HEIGHT - 2) * TIMER_WIDTH + j) as usize] = true;
            }
        ...
        pub fn get(&self, x: i32, y: i32) -> Option<bool> {
            if x >= 0 && y >= 0 && (x as u32) < TIMER_WIDTH && (y as u32) < TIMER_HEIGHT {
                Some(self.playground[(x as u32 + (y as u32) * TIMER_WIDTH) as usize])
            } else {
                None
            }
        }
        ...
        pub fn get_mut(&mut self, x: i32, y: i32) -> Option<&mut bool> {
            if x >= 0 && y >= 0 && (x as u32) < TIMER_WIDTH && (y as u32) < TIMER_HEIGHT {
                Some(&mut self.playground[(x as u32 + (y as u32) * TIMER_WIDTH) as usize])
            } else {
                None
            }
        }
...
pub fn main() -> Result<(), String> {
    ...
    let window = video_subsystem
        .window(
            "Timer",
            SQUARE_SIZE * TIMER_WIDTH,
            SQUARE_SIZE * TIMER_HEIGHT,
        )
        .position_centered()
        .build()
        .map_err(|e| e.to_string())?;
    ...
    let mut game = timer::GameOfLife::new();
    ...
    'running: loop {
        ...
        for (i, unit) in (&game).into_iter().enumerate() {
            ...
            if *unit {
                canvas.copy(
                    ...
                    None,
                    Rect::new(
                        ((i % TIMER_WIDTH) * SQUARE_SIZE) as i32,
                        ((i / TIMER_WIDTH) * SQUARE_SIZE) as i32,
                        SQUARE_SIZE,
                        SQUARE_SIZE,
                    ),
        ...
        canvas.present();
        if let timer::State::Running = game.state() {
            frame += 1;
        };
    }
